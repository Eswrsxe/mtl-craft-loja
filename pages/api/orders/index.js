...
async function createOrder(req, res, session) {
  ...
  if (couponCode) {
    ...
    if (invalidCouponMsg) {
      return res.status(400).json({ error: invalidCouponMsg });
    }
    couponDiscount =
  ...
  try {
    order = await prisma.$transaction(async (tx) => {
      // Trava a quantidade de usos do cupom de forma atômica (evita duas
      // compras simultâneas passarem do limite ao mesmo tempo).
