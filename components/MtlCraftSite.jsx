      ...
      <div className="mc-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {step === "confirm" && (
          <>
            ...
            <div className="mc-modal-ticket-msg">
              <p>
                Ao confirmar, o pedido é criado e você recebe uma DM no Discord com o
                resumo da compra e o botão para abrir o atendimento.
              </p>
            </div>
        ...
        {step === "success" && order && (
          <>
            ...
            <p className="mc-modal-desc">
              Pedido <strong>#{order.code}</strong> registrado com sucesso. Enviamos uma
              DM no seu Discord com o resumo da compra — abra o atendimento por lá para
              enviar o comprovante via PIX.
            </p>
            <div className="mc-modal-ticket-msg">
              <p>Não recebeu a DM? Verifique se suas mensagens diretas estão abertas para membros do servidor.</p>
            </div>
...
function AccountPanel({ onClose }) {
  ...
  // Se o bot estava offline quando o pedido foi criado, o cliente pode não
  // ter recebido a DM com o botão "Abrir atendimento". O pedido nunca se
  // perde (já está salvo no banco) — isso só tenta avisar o bot de novo.
  const resendToBot = async (orderId) => {
    setResendingId(orderId);
    setResendMsg("");
    try {
      await apiFetch(`/api/orders/${orderId}/notify`, { method: "POST" });
      setResendMsg("Reenviado! Confira sua DM no Discord.");
    } catch (e) {
      setResendMsg(e.message || "Não foi possível reenviar agora. Tente novamente em instantes.");
    } finally {
      setResendingId(null);
    }
  };
  ...
  return (
    <div className="mc-modal-overlay" onClick={onClose}>
      <div className="mc-modal mc-modal-account" onClick={(e) => e.stopPropagation()}>
        ...
        {errorMsg && <div className="mc-auth-error"><AlertTriangle size={14} /> {errorMsg}</div>}
        {resendMsg && <div className="mc-modal-desc">{resendMsg}</div>}
              ...
              <li key={o.id}>
                ...
                {o.status === "PENDING_PAYMENT" && (
                  <button
                    ...
                    onClick={() => resendToBot(o.id)}
                  >
                    {resendingId === o.id ? <Loader2 size={13} className="mc-spin" /> : "Reenviar para o Discord"}
                  </button>
                )}
              </li>
            ))}
